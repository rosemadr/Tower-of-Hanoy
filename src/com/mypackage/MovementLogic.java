...
//	listOfLists.add(columnC);
	...
	public List<List<Integer>> moveCard(List<List<Integer>> allLists, int moveFrom, int moveTo) throws InvalidMoveException {
		
		int cardOne = allLists.get(moveFrom).get(-1);
		Optional<Integer> cardTwo = Optional.of(allLists.get(moveTo).get(-1));
		
		if (cardOne < cardTwo.orElse(10)) {
		//checks if value in first column is < one in second, if second is empty then will equal true
		...
		return allLists; }
			...
			else { throw new InvalidMoveException("You can't move that card to that column, the card can only be placed on a card of a higher rank");
